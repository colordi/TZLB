...
- 调查导入：按事件日期从本机 PostgreSQL/PostGIS 读取事件流水中的下派与复查异常记录（春尺蠖、国槐尺蠖、美国白蛾、其他害虫、杨树食叶害虫）。
...
- `survey."美国白蛾调查表"`：美国白蛾调查表（含 `年份`、`世代` 列，用 `世代` 区分第一/二/三代）
- `survey."其他害虫调查表"`：其他害虫调查表（含 `点位名称` 列；工单候选优先读调查记录上的名称）
- `ledger."美国白蛾问题点位事件流水表"`：美国白蛾问题点位事件流水表
- `ledger."美国白蛾问题点位台账"`：美国白蛾问题点位台账
- `sites."监测点位基础表"`：监测点位基础表
- `sites."杨树点位基础表"`：杨树点位基础表（含 `当前点位状态` 字段：可调查/不可调查/伐除）
- `sites."国槐点位基础表"`：国槐点位基础表，要求 `编号` 唯一且 `geom` 不为空
- `sites."其他害虫点位基础表"`：其他害虫点位基础表
- `sites."美国白蛾点位基础表"`：美国白蛾点位基础表
- `sites."美国白蛾小区点位基础表"`：美国白蛾小区点位基础表（自 `reference."通州区小区边界"` 提取面内点，独立维护）
- `views.*`：地图展示视图，必须包含 `geom` 字段
- `views."美国白蛾调查"`：将 `sites."美国白蛾小区点位基础表"` 与美国白蛾调查记录关联为地图图层
- `views."国槐尺蠖幼虫历年发生情况"`：将 `survey."国槐尺蠖幼虫调查表"` 按年份聚合为 `年份`、`危害程度`
...
字段，会启用"调查 / 未调查"筛选；若包含 `年份`、`世代`、`危害程度`、`害虫类型`
...
1. 进入 `/workorder`。
2. 选择害虫类型和统防统治任务。
3. 点击“从数据库导入”，按事件日期从本机数据库查询当日下派与复查异常记录。
...
每条工作单记录最多上传 4 张图片。春尺蠖、国槐尺蠖、美国白蛾和其他害虫导入时会尝试按点位编号匹配
`points/杨树点位截图`、`points/国槐点位截图`、`points/美国白蛾点位截图` 或 `points/其他害虫点位截图`
下的本地截图。
