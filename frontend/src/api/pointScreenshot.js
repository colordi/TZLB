...
export async function fetchPointScreenshotBlob(pestType, code) {
  const response = await apiFetch(
    `/api/point-screenshots/preview?pest_type=${encodeURIComponent(pestType)}&code=${encodeURIComponent(code)}`,
  );
  await ensureApiSuccess(response);
