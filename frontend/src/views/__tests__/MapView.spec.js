...
describe("MapView", () => {
  ...
  it("Android 端外链使用 geo: URI 唤起系统地图选择器", async () => {
    const targetFeature = {
      type: "Feature",
      properties: {
        编号: "TY001",
    ...
    try {
      ...
      const href = wrapper.get('[data-testid="external-map-link"]').attributes("href");
      expect(href).toBe(
        `geo:0,0?q=39.8,116.6(${encodeURIComponent("通运家园")})`,
      );
    } finally {
      Object.defineProperty(window.navigator, "userAgent", {
        value: originalUA,
  ...
  it("iOS 端外链走高德 URI 以尝试拉起高德 App", async () => {
    const targetFeature = {
      type: "Feature",
      properties: {
        编号: "TY001",
    ...
    try {
      ...
      expect(href).toContain("https://uri.amap.com/marker?");
      expect(href).toContain("position=116.6,39.8");
      expect(href).toContain("callnative=1");
    } finally {
      Object.defineProperty(window.navigator, "userAgent", {
        value: originalUA,
        configurable: true,
