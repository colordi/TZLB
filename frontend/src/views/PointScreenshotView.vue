...
<template>
  <section class="point-screenshot-page mx-auto flex w-full max-w-6xl flex-col gap-4">
    <RouterLink
      class="point-screenshot-back inline-flex w-fit items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground"
      to="/workorder"
    >
    ...
    <header class="point-screenshot-head space-y-1">
      <p class="point-screenshot-eyebrow text-[10px] font-bold tracking-[0.12em] text-primary">
        POINT SCREENSHOT LIBRARY
      </p>
      <h1 class="text-2xl font-bold tracking-tight md:text-3xl">点位截图管理</h1>
      <p class="text-sm text-muted-foreground">按害虫类型查看点位截图状态，并上传、替换或删除截图。</p>
    </header>
